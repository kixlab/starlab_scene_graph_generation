...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    if prediction.get_field('pred_rel_scores').dim() == 2:
        local_container['rel_scores'] = prediction.get_field('pred_rel_scores').detach().cpu().numpy()          # (#pred_rels, num_pred_class)
    elif prediction.get_field('pred_rel_scores').dim() == 1: # use the predicate score to recover the probability after softmax
        rel_scores = prediction.get_field('pred_rel_scores').detach().cpu().numpy()
        rel_classes = prediction.get_field('pred_rel_labels').detach().cpu().numpy().astype(int)
        rel_probabilities = np.zeros((rel_scores.shape[0], 51))
        rel_probabilities[np.arange(rel_scores.shape[0]), rel_classes] = rel_scores
        local_container['rel_scores'] = rel_probabilities
