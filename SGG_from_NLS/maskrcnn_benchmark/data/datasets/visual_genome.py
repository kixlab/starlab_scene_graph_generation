...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        # self.custom_eval = True; custom_path='/hdd/skchoi/SGG_from_NLS/datasets/vg/VG_10' # Delete
        print("~~~"*10, "\nself.custom_eval:", self.custom_eval)
        if self.custom_eval:
            print("\nGet Custom images!!!! \n", custom_path)
            ...
            if self.split == 'train' and self.use_cap_trip:  # only use caption triplets during training 
                ...
                # print("=="*20); print(self.img_info); print("=="*20)
                # print(self.split_mask, len(self.split_mask))

                ## Masking index based on the split(train, val, test)
                # self.filenames = [self.filenames[i] for i in np.where(self.split_mask)[0]]
                # self.img_info = [self.img_info[i] for i in np.where(self.split_mask)[0]]
        ...
        if type(self.det_path) is str:  # single dataset
            ...
            if not (self.det_path == 'none'):
                print("self.det_path: ", self.det_path)
