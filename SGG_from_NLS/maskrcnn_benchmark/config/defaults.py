...
_C.WSVL = CN()
# Flag whether use detection results from an offline object detector (e.g., OID)
_C.WSVL.OFFLINE_OD = False
_C.WSVL.OFFLINE_OD_TYPE = 'OID'
# Flag whether use UNITER as relation predictor 
_C.WSVL.USE_UNITER = False
# The dict path, match predicted objects with gt objects using a dictionary (hard mapping)
_C.WSVL.HARD_DICT_MATCH = None
# Flag to use caption triplets as supervision instead of human annotated scene graph labels
_C.WSVL.USE_CAP_TRIP = False
# The dict path, provide triplet labels parsed from caption dataset
_C.WSVL.CAP_TRIP_LABEL = None
# The dict path, provide class-wise weights for loss computation
_C.WSVL.LOSS_CLS_WEIGHTS = None
# The dict path, convert the predicted objects/predicates (caption categories) into VG 150-50 categories
_C.WSVL.CAP_VG_DICT = None
# Flag whether skip training and evaluate model directly
_C.WSVL.SKIP_TRAIN = False
